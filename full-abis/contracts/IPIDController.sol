...
interface IPIDController is IAuthorizable, IModifiable {
  ...
  struct ControllerGains {
    ...
    int256 /* WAD */ ki;
    // This value determines how much the market price influences the rate adjustment
    int256 /* WAD */ influenceFactor;
  }
   ...
   * @return _influenceFactor This value determines how much the market price influences the rate adjustment
   */
  // solhint-disable-next-line private-vars-leading-underscore
  function _controllerGains() external view returns (int256 _kp, int256 _ki, int256 _influenceFactor);
   ...
   * @param  _targetPrice The target price derived from the carbon price index oracle
   * @return _redemptionRate The computed redemption rate that will adjust the redemption price towards the target
   */
  function computeRate(uint256 _marketPrice, uint256 _targetPrice) external returns (uint256 _redemptionRate);
  ...
  function getGainAdjustedTerms(
    ...
    external
    view
    returns (int256 _proportionalGain, int256 _integralGain);
  ...
  function getNextDeviationCumulative(
    ...
    external
    returns (int256 _priceDeviationCumulative, int256 _timeAdjustedDeviation);

  /**
   * @notice Returns whether the P + I sum exceeds the noise barrier
   * @param  _piSum Represents a sum between P + I
   * @param  _targetPrice The target price from the carbon price index oracle
   * @return _breaksNb Whether the P + I sum exceeds the noise barrier
   */
  function breaksNoiseBarrier(uint256 _piSum, uint256 _targetPrice) external view returns (bool _breaksNb);
