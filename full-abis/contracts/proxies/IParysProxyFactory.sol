...
import {IParysProxy} from '@interfaces/proxies/IParysProxy.sol';

interface IParysProxyFactory {
  // --- Events ---
  ...
  error AlreadyHasProxy(address _owner, IParysProxy _proxy);
  ...
  function proxies(
    address _owner
  ) external view returns (IParysProxy _proxy);
  ...
  function build(
    address _owner
  ) external returns (address payable _proxy);
}
