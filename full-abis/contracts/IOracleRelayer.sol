...
interface IOracleRelayer is IAuthorizable, IDisableable, IModifiable, IModifiablePerCollateral {
  ...
  /**
   * @notice Emitted when the target redemption price is updated from the oracle
   * @param _targetPrice The new target redemption price [ray]
   */
  event UpdateTargetRedemptionPrice(uint256 _targetPrice);
  ...
  /**
   * @notice The oracle used to provide the redemption price target based on the carbon price index
   * @return _carbonPriceIndexOracle Address of the contract that provides the carbon price index
   */
  function carbonPriceIndexOracle() external view returns (IBaseOracle _carbonPriceIndexOracle);
  ...
  /**
   * @notice Returns the current target redemption price from the oracle
   * @return _target The current target price that the system is trying to approach [ray]
   */
  function targetRedemptionPrice() external view returns (uint256 _target);

  /**
   * @notice Updates the target redemption price based on the carbon price index oracle
   * @return _updatedTarget The updated target redemption price [ray]
   * @dev    This no longer directly updates the redemption price, but instead sets a target
   *         that the redemption price will gradually approach through rate adjustments
   */
  function updateRedemptionPriceFromOracle() external returns (uint256 _updatedTarget);
