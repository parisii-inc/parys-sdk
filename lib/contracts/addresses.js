...
exports.debugGetTokenAddresses = exports.debugGetAddresses = exports.getAddressList = exports.getTokenList = exports.isBrowser = void 0;
 ...
 */
function loadLocalContractAddresses() {
    try {
        ...
        if (exports.isBrowser) {
            ...
            var filePath = '/data/31337-contracts.json';
            ...
            try {
                ...
                xhr.send(null);
                if (xhr.status === 200) {
                    var contractAddresses = JSON.parse(xhr.responseText);
                    //console.log('[SDK] Successfully loaded contract addresses')
                    return contractAddresses;
                }
                else {
                    console.error("[SDK] Failed to load contract addresses: ".concat(xhr.status, " ").concat(xhr.statusText));
                }
            }
            catch (error) {
...
function loadLocalTokenList() {
    try {
        ...
        if (exports.isBrowser) {
            // Standard path for local development
            var filePath = '/data/31337-tokens.json';
            ...
            try {
                ...
                xhr.send(null);
                if (xhr.status === 200) {
                    var tokenList = JSON.parse(xhr.responseText);
                    //console.log('[SDK] Successfully loaded token list')
                    return tokenList;
                }
                else {
                    console.error("[SDK] Failed to load token list: ".concat(xhr.status, " ").concat(xhr.statusText));
                }
            }
            catch (error) {
            ...
            // In Node.js environment, try a relative path
            var filePath = './public/data/31337-tokens.json';
            var resolvedPath = path.resolve(process.cwd(), filePath);
            if (fs.existsSync(resolvedPath)) {
                var fileContent = fs.readFileSync(resolvedPath, { encoding: 'utf8' });
                var tokenList = JSON.parse(fileContent);
...
var getTokenList = function (network) {
    if (network === 'localnet') {
        //console.log('[SDK] Getting tokens for localnet')
        // Try to load from local file
        var localTokens = loadLocalTokenList();
        if (localTokens) {
...
var getAddressList = function (network) {
    if (network === 'localnet') {
        //console.log('[SDK] Getting contract addresses for localnet')
        // Try to load from local file
        var localAddresses = loadLocalContractAddresses();
        if (localAddresses) {
...
function debugGetAddresses(networkId) {
...
exports.debugGetAddresses = debugGetAddresses;
function debugGetTokenAddresses(networkId) {
...
exports.debugGetTokenAddresses = debugGetTokenAddresses;
