/* Autogenerated file. Do not edit manually. */
/* tslint:disable */
/* eslint-disable */

import { Contract, Signer, utils } from "ethers";
import type { Provider } from "@ethersproject/providers";
import type {
  GlobalSettlement,
  GlobalSettlementInterface,
  IGlobalSettlement,
} from "../GlobalSettlement";

const _abi = [
  {
    type: "constructor",
    inputs: [
      {
        name: "_safeEngine",
        type: "address",
        internalType: "address",
      },
      {
        name: "_liquidationEngine",
        type: "address",
        internalType: "address",
      },
      {
        name: "_oracleRelayer",
        type: "address",
        internalType: "address",
      },
      {
        name: "_coinJoin",
        type: "address",
        internalType: "address",
      },
      {
        name: "_collateralJoinFactory",
        type: "address",
        internalType: "address",
      },
      {
        name: "_collateralAuctionHouseFactory",
        type: "address",
        internalType: "address",
      },
      {
        name: "_stabilityFeeTreasury",
        type: "address",
        internalType: "address",
      },
      {
        name: "_accountingEngine",
        type: "address",
        internalType: "address",
      },
      {
        name: "_gsParams",
        type: "tuple",
        internalType: "struct IGlobalSettlement.GlobalSettlementParams",
        components: [
          {
            name: "shutdownCooldown",
            type: "uint256",
            internalType: "uint256",
          },
        ],
      },
    ],
    stateMutability: "nonpayable",
  },
  {
    type: "function",
    name: "_params",
    inputs: [],
    outputs: [
      {
        name: "shutdownCooldown",
        type: "uint256",
        internalType: "uint256",
      },
    ],
    stateMutability: "view",
  },
  {
    type: "function",
    name: "accountingEngine",
    inputs: [],
    outputs: [
      {
        name: "",
        type: "address",
        internalType: "contract IDisableable",
      },
    ],
    stateMutability: "view",
  },
  {
    type: "function",
    name: "addAuthorization",
    inputs: [
      {
        name: "_account",
        type: "address",
        internalType: "address",
      },
    ],
    outputs: [],
    stateMutability: "nonpayable",
  },
  {
    type: "function",
    name: "authorizedAccounts",
    inputs: [
      {
        name: "_account",
        type: "address",
        internalType: "address",
      },
    ],
    outputs: [
      {
        name: "_authorized",
        type: "bool",
        internalType: "bool",
      },
    ],
    stateMutability: "view",
  },
  {
    type: "function",
    name: "authorizedAccounts",
    inputs: [],
    outputs: [
      {
        name: "_accounts",
        type: "address[]",
        internalType: "address[]",
      },
    ],
    stateMutability: "view",
  },
  {
    type: "function",
    name: "calculateCashPrice",
    inputs: [
      {
        name: "_cType",
        type: "bytes32",
        internalType: "bytes32",
      },
    ],
    outputs: [],
    stateMutability: "nonpayable",
  },
  {
    type: "function",
    name: "coinBag",
    inputs: [
      {
        name: "_usr",
        type: "address",
        internalType: "address",
      },
    ],
    outputs: [
      {
        name: "_wad",
        type: "uint256",
        internalType: "uint256",
      },
    ],
    stateMutability: "view",
  },
  {
    type: "function",
    name: "coinJoin",
    inputs: [],
    outputs: [
      {
        name: "",
        type: "address",
        internalType: "contract IDisableable",
      },
    ],
    stateMutability: "view",
  },
  {
    type: "function",
    name: "coinsUsedToRedeem",
    inputs: [
      {
        name: "_cType",
        type: "bytes32",
        internalType: "bytes32",
      },
      {
        name: "_usr",
        type: "address",
        internalType: "address",
      },
    ],
    outputs: [
      {
        name: "_wad",
        type: "uint256",
        internalType: "uint256",
      },
    ],
    stateMutability: "view",
  },
  {
    type: "function",
    name: "collateralAuctionHouseFactory",
    inputs: [],
    outputs: [
      {
        name: "",
        type: "address",
        internalType: "contract IDisableable",
      },
    ],
    stateMutability: "view",
  },
  {
    type: "function",
    name: "collateralCashPrice",
    inputs: [
      {
        name: "_cType",
        type: "bytes32",
        internalType: "bytes32",
      },
    ],
    outputs: [
      {
        name: "_ray",
        type: "uint256",
        internalType: "uint256",
      },
    ],
    stateMutability: "view",
  },
  {
    type: "function",
    name: "collateralJoinFactory",
    inputs: [],
    outputs: [
      {
        name: "",
        type: "address",
        internalType: "contract IDisableable",
      },
    ],
    stateMutability: "view",
  },
  {
    type: "function",
    name: "collateralShortfall",
    inputs: [
      {
        name: "_cType",
        type: "bytes32",
        internalType: "bytes32",
      },
    ],
    outputs: [
      {
        name: "_wad",
        type: "uint256",
        internalType: "uint256",
      },
    ],
    stateMutability: "view",
  },
  {
    type: "function",
    name: "collateralTotalDebt",
    inputs: [
      {
        name: "_cType",
        type: "bytes32",
        internalType: "bytes32",
      },
    ],
    outputs: [
      {
        name: "_wad",
        type: "uint256",
        internalType: "uint256",
      },
    ],
    stateMutability: "view",
  },
  {
    type: "function",
    name: "contractEnabled",
    inputs: [],
    outputs: [
      {
        name: "",
        type: "bool",
        internalType: "bool",
      },
    ],
    stateMutability: "view",
  },
  {
    type: "function",
    name: "disableContract",
    inputs: [],
    outputs: [],
    stateMutability: "nonpayable",
  },
  {
    type: "function",
    name: "fastTrackAuction",
    inputs: [
      {
        name: "_cType",
        type: "bytes32",
        internalType: "bytes32",
      },
      {
        name: "_auctionId",
        type: "uint256",
        internalType: "uint256",
      },
    ],
    outputs: [],
    stateMutability: "nonpayable",
  },
  {
    type: "function",
    name: "finalCoinPerCollateralPrice",
    inputs: [
      {
        name: "_cType",
        type: "bytes32",
        internalType: "bytes32",
      },
    ],
    outputs: [
      {
        name: "_ray",
        type: "uint256",
        internalType: "uint256",
      },
    ],
    stateMutability: "view",
  },
  {
    type: "function",
    name: "freeCollateral",
    inputs: [
      {
        name: "_cType",
        type: "bytes32",
        internalType: "bytes32",
      },
    ],
    outputs: [],
    stateMutability: "nonpayable",
  },
  {
    type: "function",
    name: "freezeCollateralType",
    inputs: [
      {
        name: "_cType",
        type: "bytes32",
        internalType: "bytes32",
      },
    ],
    outputs: [],
    stateMutability: "nonpayable",
  },
  {
    type: "function",
    name: "liquidationEngine",
    inputs: [],
    outputs: [
      {
        name: "",
        type: "address",
        internalType: "contract ILiquidationEngine",
      },
    ],
    stateMutability: "view",
  },
  {
    type: "function",
    name: "modifyParameters",
    inputs: [
      {
        name: "_param",
        type: "bytes32",
        internalType: "bytes32",
      },
      {
        name: "_data",
        type: "bytes",
        internalType: "bytes",
      },
    ],
    outputs: [],
    stateMutability: "nonpayable",
  },
  {
    type: "function",
    name: "oracleRelayer",
    inputs: [],
    outputs: [
      {
        name: "",
        type: "address",
        internalType: "contract IOracleRelayer",
      },
    ],
    stateMutability: "view",
  },
  {
    type: "function",
    name: "outstandingCoinSupply",
    inputs: [],
    outputs: [
      {
        name: "",
        type: "uint256",
        internalType: "uint256",
      },
    ],
    stateMutability: "view",
  },
  {
    type: "function",
    name: "params",
    inputs: [],
    outputs: [
      {
        name: "_globalSettlementParams",
        type: "tuple",
        internalType: "struct IGlobalSettlement.GlobalSettlementParams",
        components: [
          {
            name: "shutdownCooldown",
            type: "uint256",
            internalType: "uint256",
          },
        ],
      },
    ],
    stateMutability: "view",
  },
  {
    type: "function",
    name: "prepareCoinsForRedeeming",
    inputs: [
      {
        name: "_coinAmount",
        type: "uint256",
        internalType: "uint256",
      },
    ],
    outputs: [],
    stateMutability: "nonpayable",
  },
  {
    type: "function",
    name: "processSAFE",
    inputs: [
      {
        name: "_cType",
        type: "bytes32",
        internalType: "bytes32",
      },
      {
        name: "_safe",
        type: "address",
        internalType: "address",
      },
    ],
    outputs: [],
    stateMutability: "nonpayable",
  },
  {
    type: "function",
    name: "redeemCollateral",
    inputs: [
      {
        name: "_cType",
        type: "bytes32",
        internalType: "bytes32",
      },
      {
        name: "_coinsAmount",
        type: "uint256",
        internalType: "uint256",
      },
    ],
    outputs: [],
    stateMutability: "nonpayable",
  },
  {
    type: "function",
    name: "removeAuthorization",
    inputs: [
      {
        name: "_account",
        type: "address",
        internalType: "address",
      },
    ],
    outputs: [],
    stateMutability: "nonpayable",
  },
  {
    type: "function",
    name: "safeEngine",
    inputs: [],
    outputs: [
      {
        name: "",
        type: "address",
        internalType: "contract ISAFEEngine",
      },
    ],
    stateMutability: "view",
  },
  {
    type: "function",
    name: "setOutstandingCoinSupply",
    inputs: [],
    outputs: [],
    stateMutability: "nonpayable",
  },
  {
    type: "function",
    name: "shutdownSystem",
    inputs: [],
    outputs: [],
    stateMutability: "nonpayable",
  },
  {
    type: "function",
    name: "shutdownTime",
    inputs: [],
    outputs: [
      {
        name: "",
        type: "uint256",
        internalType: "uint256",
      },
    ],
    stateMutability: "view",
  },
  {
    type: "function",
    name: "stabilityFeeTreasury",
    inputs: [],
    outputs: [
      {
        name: "",
        type: "address",
        internalType: "contract IDisableable",
      },
    ],
    stateMutability: "view",
  },
  {
    type: "event",
    name: "AddAuthorization",
    inputs: [
      {
        name: "_account",
        type: "address",
        indexed: false,
        internalType: "address",
      },
    ],
    anonymous: false,
  },
  {
    type: "event",
    name: "CalculateCashPrice",
    inputs: [
      {
        name: "_cType",
        type: "bytes32",
        indexed: true,
        internalType: "bytes32",
      },
      {
        name: "_collateralCashPrice",
        type: "uint256",
        indexed: false,
        internalType: "uint256",
      },
    ],
    anonymous: false,
  },
  {
    type: "event",
    name: "DisableContract",
    inputs: [],
    anonymous: false,
  },
  {
    type: "event",
    name: "FastTrackAuction",
    inputs: [
      {
        name: "_cType",
        type: "bytes32",
        indexed: true,
        internalType: "bytes32",
      },
      {
        name: "_auctionId",
        type: "uint256",
        indexed: true,
        internalType: "uint256",
      },
      {
        name: "_collateralTotalDebt",
        type: "uint256",
        indexed: false,
        internalType: "uint256",
      },
    ],
    anonymous: false,
  },
  {
    type: "event",
    name: "FreeCollateral",
    inputs: [
      {
        name: "_cType",
        type: "bytes32",
        indexed: true,
        internalType: "bytes32",
      },
      {
        name: "_sender",
        type: "address",
        indexed: true,
        internalType: "address",
      },
      {
        name: "_collateralAmount",
        type: "uint256",
        indexed: false,
        internalType: "uint256",
      },
    ],
    anonymous: false,
  },
  {
    type: "event",
    name: "FreezeCollateralType",
    inputs: [
      {
        name: "_cType",
        type: "bytes32",
        indexed: true,
        internalType: "bytes32",
      },
      {
        name: "_finalCoinPerCollateralPrice",
        type: "uint256",
        indexed: false,
        internalType: "uint256",
      },
    ],
    anonymous: false,
  },
  {
    type: "event",
    name: "ModifyParameters",
    inputs: [
      {
        name: "_param",
        type: "bytes32",
        indexed: true,
        internalType: "bytes32",
      },
      {
        name: "_cType",
        type: "bytes32",
        indexed: true,
        internalType: "bytes32",
      },
      {
        name: "_data",
        type: "bytes",
        indexed: false,
        internalType: "bytes",
      },
    ],
    anonymous: false,
  },
  {
    type: "event",
    name: "PrepareCoinsForRedeeming",
    inputs: [
      {
        name: "_sender",
        type: "address",
        indexed: true,
        internalType: "address",
      },
      {
        name: "_coinBag",
        type: "uint256",
        indexed: false,
        internalType: "uint256",
      },
    ],
    anonymous: false,
  },
  {
    type: "event",
    name: "ProcessSAFE",
    inputs: [
      {
        name: "_cType",
        type: "bytes32",
        indexed: true,
        internalType: "bytes32",
      },
      {
        name: "_safe",
        type: "address",
        indexed: true,
        internalType: "address",
      },
      {
        name: "_collateralShortfall",
        type: "uint256",
        indexed: false,
        internalType: "uint256",
      },
    ],
    anonymous: false,
  },
  {
    type: "event",
    name: "RedeemCollateral",
    inputs: [
      {
        name: "_cType",
        type: "bytes32",
        indexed: true,
        internalType: "bytes32",
      },
      {
        name: "_sender",
        type: "address",
        indexed: true,
        internalType: "address",
      },
      {
        name: "_coinsAmount",
        type: "uint256",
        indexed: false,
        internalType: "uint256",
      },
      {
        name: "_collateralAmount",
        type: "uint256",
        indexed: false,
        internalType: "uint256",
      },
    ],
    anonymous: false,
  },
  {
    type: "event",
    name: "RemoveAuthorization",
    inputs: [
      {
        name: "_account",
        type: "address",
        indexed: false,
        internalType: "address",
      },
    ],
    anonymous: false,
  },
  {
    type: "event",
    name: "SetOutstandingCoinSupply",
    inputs: [
      {
        name: "_outstandingCoinSupply",
        type: "uint256",
        indexed: false,
        internalType: "uint256",
      },
    ],
    anonymous: false,
  },
  {
    type: "event",
    name: "ShutdownSystem",
    inputs: [],
    anonymous: false,
  },
  {
    type: "error",
    name: "AlreadyAuthorized",
    inputs: [],
  },
  {
    type: "error",
    name: "ContractIsDisabled",
    inputs: [],
  },
  {
    type: "error",
    name: "ContractIsEnabled",
    inputs: [],
  },
  {
    type: "error",
    name: "GS_CollateralCashPriceAlreadyDefined",
    inputs: [],
  },
  {
    type: "error",
    name: "GS_CollateralCashPriceNotDefined",
    inputs: [],
  },
  {
    type: "error",
    name: "GS_FinalCollateralPriceAlreadyDefined",
    inputs: [],
  },
  {
    type: "error",
    name: "GS_FinalCollateralPriceNotDefined",
    inputs: [],
  },
  {
    type: "error",
    name: "GS_InsufficientBagBalance",
    inputs: [],
  },
  {
    type: "error",
    name: "GS_OutstandingCoinSupplyNotZero",
    inputs: [],
  },
  {
    type: "error",
    name: "GS_OutstandingCoinSupplyZero",
    inputs: [],
  },
  {
    type: "error",
    name: "GS_SafeDebtNotZero",
    inputs: [],
  },
  {
    type: "error",
    name: "GS_ShutdownCooldownNotFinished",
    inputs: [],
  },
  {
    type: "error",
    name: "GS_SurplusNotZero",
    inputs: [],
  },
  {
    type: "error",
    name: "IntOverflow",
    inputs: [],
  },
  {
    type: "error",
    name: "NoCode",
    inputs: [
      {
        name: "_contract",
        type: "address",
        internalType: "address",
      },
    ],
  },
  {
    type: "error",
    name: "NonDisableable",
    inputs: [],
  },
  {
    type: "error",
    name: "NotAuthorized",
    inputs: [],
  },
  {
    type: "error",
    name: "NullAddress",
    inputs: [],
  },
  {
    type: "error",
    name: "Unauthorized",
    inputs: [],
  },
  {
    type: "error",
    name: "UnrecognizedCType",
    inputs: [],
  },
  {
    type: "error",
    name: "UnrecognizedParam",
    inputs: [],
  },
] as const;

export class GlobalSettlement__factory {
  static readonly abi = _abi;
  static createInterface(): GlobalSettlementInterface {
    return new utils.Interface(_abi) as GlobalSettlementInterface;
  }
  static connect(
    address: string,
    signerOrProvider: Signer | Provider
  ): GlobalSettlement {
    return new Contract(address, _abi, signerOrProvider) as GlobalSettlement;
  }
}
